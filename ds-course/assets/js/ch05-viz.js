...
(function () {
  ...
  (function bmBadChar() {
    ...
    var S = "HERE IS A SIMPLE EXAMPLE", P = "EXAMPLE";
    var n = S.length, m = P.length;
    var frames = [], shiftCount = 0;
    ...
    while (align + m - 1 < n) {
      ...
      var bad = S[align + j];
      var rm = last[bad] === undefined ? -1 : last[bad];
      var move = j - rm;
  ...
  (function bmGoodSuffix() {
    ...
    while (align <= n - m && guard++ < 20) {
      ...
      var gs = goodSuffixShift(P, j);
      var bad = S[align + j];
      var rm2 = last2[bad] === undefined ? -1 : last2[bad];
      var bc = j - rm2; if (bc <= 0) bc = 1;
      if (suffixLen > 0) {
