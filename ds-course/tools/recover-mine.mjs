...
const dir = fs.readdirSync(SESS).find(d => d.startsWith("session-"));
const tmp = path.join(os.tmpdir(), "mine.jsonl");
...
const WRITE = { write: 1, edit: 1 };
function collect(v, out, seen) {
  ...
  if (typeof v.name === "string" && v.arguments !== undefined && WRITE[v.name]) {
    ...
    if (a && typeof a === "object" && String(a.file_path || a.path || "").includes(key)) {
      const id = v.id || JSON.stringify(a).slice(0, 150);
...
const outFile = path.join(os.tmpdir(), "ds-mine", key);
