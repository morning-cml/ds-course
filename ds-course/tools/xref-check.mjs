...
const VALID_NO = new Set(PAGES.map(p => p.no));
...
let errors = 0, warnings = 0, total = 0, filesChecked = 0;
const errs = [], warns = [];

for (const t of targets) {
  const text = fs.readFileSync(t.full, "utf8");
  filesChecked++;
  const re = /第\s*(\d{1,2})\s*讲/g;
  let m;
  while ((m = re.exec(text)) !== null) {
    const no = Number(m[1]);
    total++;

    const after = phraseAfter(text, re.lastIndex);
    const beforeRaw = text.slice(Math.max(0, m.index - 18), m.index).replace(/<[^>]*>/g, "");
    ...
    const hits = KEYWORDS.filter(k => k.re.test(after));
    if (!hits.length) continue;

    const selfHit = hits.some(h => h.no === no);
    const other = hits.filter(h => h.no !== no);
