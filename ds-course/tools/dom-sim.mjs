...
function makeEnv(pageFile) {
  ...
  const window = {
    document,
    location: { href: "file:///" + pageFile, pathname: pageFile },
    navigator: { clipboard: null, userAgent: "node-dom-sim" },
    innerWidth: 1400,
    addEventListener(t, fn) { (this._l = this._l || {})[t] = (this._l[t] || []).concat(fn); },
    removeEventListener() {},
    setTimeout: (fn, ms) => 0,          // 不真正调度，避免异步干扰
    clearTimeout() {},
    setInterval: (fn, ms) => { (window.__intervals = window.__intervals || []).push(fn); return 1; },
    clearInterval() {},
    print() {},
  ...
  return { window, document, errors, errors2 };
}
...
function runPage(page) {
  ...
  const ids = [...html.matchAll(/id\s*=\s*"([^"]+)"/g)].map(m => m[1]);
  ...
  const mkEl = (tag, cls, id) => {
    const e = window.document.createElement(tag);
    if (cls) e.className = cls;
    if (id) e.setAttribute("id", id);
    return e;
  };
  const topbar = mkEl("header", "topbar");
  const sidebar = mkEl("aside", "sidebar toc-auto");
  const content = mkEl("main", "content");
  const layout = mkEl("div", "layout");
  ...
  /* ---- 逐帧渲染所有动画 ---- */
  const vizRoots = queryAll(window.document, ".viz");
  let framesTotal = 0, framesFailed = 0, emptyFrames = 0, noDesc = 0, emptyStage = 0;
  const failDetail = [];
  const perViz = [];
  for (const v of vizRoots) {
    const list = v._vizList && v._vizList.length ? v._vizList : (v._viz ? [v._viz] : []);
  ...
  const mounted = vizRoots.filter(v => v.querySelector(".viz-stage")).length;
  const empty = vizIds.filter(id => {
  ...
  if (drawer) {
    ...
    if (btnToc) {
      const T = (name, cond, detail) => tocTests.push([name, detail, cond]);
      const W = window.innerWidth;                    // 仿真视口宽度
      const pin = queryAll(window.document, ".toc-pin")[0];
      const moveTo = (x) => window.fire ? window.fire("mousemove", { clientX: x })
                                        : (window._l.mousemove || []).forEach(f => f({ clientX: x }));
      const isOpen = () => bodyEl.classList.contains("toc-hover");
      const drawerLeft = W - Math.min(330, W * 0.88);
      ...
      const tab = queryAll(window.document, ".toc-tab")[0];
      if (tab) {
      ...
      // 抽屉的「✕」按钮
      const closeBtn = queryAll(window.document, ".toc-close")[0];
      if (closeBtn) {
        closeBtn.click();
...
for (const p of pages) {
  ...
  const ok = r.runtimeErrors.length === 0 && r.emptyViz.length === 0 && r.framesFailed === 0 &&
             r.emptyFrames === 0 && r.emptyStage === 0 &&
             r.tocTests.every(t => t[2]);
  if (!ok) bad++;
  console.log(
