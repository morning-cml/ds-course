 ...
 *
 * 退出码：2 = 参数不全；1 = 页面里找不到该 data-file 的代码块；
 *        0 = 成功（不加 --apply 时只预览，不写盘）
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = path.resolve(import.meta.dirname, "..");
const [page, fileKey, codeFile] = process.argv.slice(2).filter(a => a !== "--apply");   // 位置参数：页面 / 块的 data-file 名 / 新代码文件
const APPLY = process.argv.includes("--apply");                                        // 加 --apply 才写回页面
if (!page || !fileKey || !codeFile) {
  console.error("用法: node tools/replace-block.mjs <页面> <data-file名> <新代码文件> [--apply]");
  process.exit(2);
}

const esc = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");   // HTML 转义（注意 & 必须最先替换）

const pagePath = path.join(ROOT, page);
const html = fs.readFileSync(pagePath, "utf8");   // 目标页面原文
const newCode = fs.readFileSync(codeFile, "utf8").replace(/\r\n/g, "\n").replace(/^\n+|\s+$/g, "");   // 新代码（统一 LF 并去首尾空白）

/* 定位该 data-file 对应的 <pre> 块（fileKey 里可能有正则元字符，先转义） */
...
if (!m) {
  ...
  const m2 = html.match(alt);   // 备用匹配结果
  if (!m2) { console.error("找不到 data-file=\"" + fileKey + "\" 的代码块"); process.exit(1); }
  const head = m2[0].slice(0, m2[0].indexOf(">") + 1);   // 保留原有的 <pre ...> 开标签（data-file 等属性不动）
  const replaced = head + "\n" + esc(newCode) + "\n</pre>";
  const out = html.replace(m2[0], replaced);
  finish(out, m2[0].length, replaced.length);
} else {
  const head = m[0].slice(0, m[0].indexOf(">") + 1);   // 同上：只换内容，开标签原样保留
  const replaced = head + "\n" + esc(newCode) + "\n</pre>";
  const out = html.replace(m[0], replaced);
  finish(out, m[0].length, replaced.length);
}

function finish(out, oldLen, newLen) {   // out = 替换后的整页内容，oldLen/newLen = 被替换块的新旧长度
