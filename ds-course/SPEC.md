...
| 09 | 9.9 | 导航路线规划（Dijkstra / A* / 层次图预处理）、网络路由协议（OSPF 的 Dijkstra、RIP 的 Bellman-Ford）、任务调度与构建系统（拓扑排序）、项目管理关键路径（CPM / PERT） |
| 10 | 10.8 | 哈希冲突方案在真实实现里怎么选（链地址法为主）、负载因子与扩容、`std::unordered_map` 的桶与防卡哈希、数据库索引为何不用哈希（不支持范围查询）、布隆过滤器 / 一致性哈希 / 去重场景 |
