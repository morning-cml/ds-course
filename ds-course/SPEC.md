...
> 所有章节页面必须严格遵守本规范，以保证 16 个页面视觉与交互完全统一、可直接离线打开。

## 0. 绝对规则

1. **写章节时不要去改 `assets/js/course.js`**，只用其中已有的能力（`DS.Viz` / `DS.SVG` 等）。
   它是全站 17 个页面（16 讲 + 总览页）共用的引擎，改一处影响全站：只有修公共 bug（例如导航抽屉、代码高亮）才动它，
...
4. 页面必须能**纯离线双击打开**：不允许 CDN、不允许外链 JS/CSS/图片、不允许 `fetch`。所有图形用**内联 SVG** 或 Canvas 现场绘制。
   指向外部网页的普通超链接 `<a href="https://…">`（例如第 14 讲的洛谷题目链接）不算资源，允许使用——离线时只是点不开，页面照常显示。
...
「所有动画每一帧真正画出来的内容（文字 + 颜色类）」算成一个总指纹。改动前后各跑一次，
